...
log_run_event(conn, run_id, "raw_dashboard_properties", "started",
...
tryCatch({
  ...
  log_run_event(conn, run_id, "raw_dashboard_properties", "success",
    message_text     = paste("Properties table updated with", nrow(account_list), "rows"),
    rows_added       = nrow(account_list),
    duration_seconds = as.double(difftime(Sys.time(), start_time, units = "secs"))
  )
}, error = function(e) {
  log_run_event(conn, run_id, "raw_dashboard_properties", "error",
